...
def main():
    ...
    if trainINFO['ISTRUE']:
        ...
        print('python ../tracking/train_sot.py --cfg {0} 2>&1 | tee logs/{1}'.format(args.cfg, train_log_name))

        if not DDP:
            os.system('python ../tracking/train_sot.py --cfg {0}  | tee logs/{1}'.format(args.cfg, train_log_name))
        else:
            print('====> use DDP for training <====')
            gpu_nums = len([int(i) for i in commonINFO.GPUS.split(',')])
            os.system(
                'python -m torch.distributed.launch --nproc_per_node={0} ../tracking/train_sot.py --cfg {1}  | tee logs/{2}'
