    ...
    def run(self, dataset='TNL2K', result_path='./result/', tune=False, tracker_reg='Ocean*', start=0, end=1e6, trackers=None):
        ...
        if not tune:
            ...
            if self.video is None:
                ...
                success_overlap = np.zeros((1, len(trackers), len(thresholds_overlap)))
                #success_error = np.zeros((1, len(trackers), len(thresholds_error)))
        else:
            if self.video is None:
                success_overlap = np.zeros((n_seq, 1, len(thresholds_overlap)))
                #success_error = np.zeros((n_seq, 1, len(thresholds_error)))
            else:
                success_overlap = np.zeros((1, 1, len(thresholds_overlap)))
                #success_error = np.zeros((1, 1, len(thresholds_error)))
            ...
            if self.video is None or (self.video is not None and self.video == seq):  # stq
                ...
                #        print('{:d} processing:{} tracker: {}'.format(i, seq, tracker))
                        ...
                        if self.video is None:
                            ...
                            success_overlap[0][j] = self.compute_success_overlap(gt_rect, bb, n_frame=n_frame)
                            #success_error[0][j] = self.compute_success_error(gt_center, center, n_frame=n_frame)
                else:
                    ...
                    success_overlap[i][0] = self.compute_success_overlap(gt_rect, bb, n_frame=n_frame)
                #    success_error[i][0] = self.compute_success_error(gt_center, center, n_frame=n_frame)
