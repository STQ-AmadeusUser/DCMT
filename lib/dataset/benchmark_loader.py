...
class load_sot_benchmark():
    ...
    def load(self):
        ...
        elif 'UAV10FPS' in self.dataset:
            return self.load_UAV10FPS()
        elif 'UAV20L' in self.dataset:
            return self.load_UAV20L()
        elif 'UAVDT' in self.dataset:
            return self.load_UAVDT()
        elif 'UAVTRACK112' == self.dataset:
            return self.load_UAVTRACK112()
        elif 'UAVTRACK112L' == self.dataset:
            return self.load_UAVTRACK112L()
        elif 'UAVDARK135' == self.dataset:
            return self.load_UAVDARK135()
        elif 'VISDRONE' == self.dataset:
            return self.load_VISDRONE()
        elif 'DTB70' in self.dataset:
            return self.load_DTB70()
        elif 'VTUAVL' in self.dataset:
            return self.load_VTUAVL()
        elif 'VTUAVS' in self.dataset:
            return self.load_VTUAVS()
        elif 'ANTIUAV' in self.dataset:
            return self.load_ANTIUAV()
        elif 'DARKTRACK' in self.dataset:
            return self.load_DARKTRACK()
    ...
    def load_OTB(self):
        ...
        for v in info.keys():
            ...
            # info[v]['gt'] = np.array(info[v]['gt_rect'])  # for SiamGAT test
            info[v]['name'] = v
    ...
    def load_VTUAVL(self):
        ...
        for v in info.keys():
            path_name = info[v]['name']
            info[v]['image_files'] = [join(base_path, path_name, 'rgb', im_f) for im_f in info[v]['image_files']]
            info[v]['gt'] = np.array(info[v]['gt_rect'])
            info[v]['name'] = v

        return info

    def load_VTUAVS(self):
        """
        OTB: http://cvlab.hanyang.ac.kr/tracker_benchmark/datasets.html
        """
        info = {}
    ...
    def load_LASOT(self):
        ...
        for video in videos:
            ...
            if video not in testingvideos:  # 280 testing videos
                continue
    ...
    def load_UAV10FPS(self):
        ...
        for video in videos:
            ...
            # get img file
            imgs = vinfo['img_names']
            image_files = [join(base_path, 'data_seq/UAV123_10fps/', im) for im in imgs]
            info[video] = {'image_files': image_files, 'gt': gt, 'name': video}

        return info

    def load_UAV20L(self):
        info = {}
        base_path = join(realpath(dirname(__file__)), '../../dataset', 'UAV123')
        json_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset + '.json')
        jsons = json.load(open(json_path, 'r'))
        videos = list(jsons.keys())
    ...
    def load_DARKTRACK(self):
        ...
        for video in videos:
            ...
            # get img file
            imgs = vinfo['img_names']
            image_files = [join(base_path, 'data_seq/', im) for im in imgs]
            info[video] = {'image_files': image_files, 'gt': gt, 'name': video}

        return info

    def load_UAVDARK135(self):
        info = {}
        base_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset)
        json_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset + '.json')
        jsons = json.load(open(json_path, 'r'))
    ...
    def load_UAVDT(self):
        info = {}
        base_path = join(realpath(dirname(__file__)), '../../dataset', 'UAVDT')
        json_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset + '.json')
        jsons = json.load(open(json_path, 'r'))
        videos = list(jsons.keys())
        ...
        for video in videos:
            ...
            # get img file
            imgs = vinfo['img_names']
            image_files = [join(base_path, 'UAV-benchmark-S/', im) for im in imgs]
            info[video] = {'image_files': image_files, 'gt': gt, 'name': video}

        return info

    def load_ANTIUAV(self):
        info = {}
        base_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset)
        json_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset + '.json')
        jsons = json.load(open(json_path, 'r'))
        ...
        for video in videos:
            ...
            # get img file
            imgs = vinfo['img_names']
            image_files = [join(base_path, 'sequences/', im) for im in imgs]
            info[video] = {'image_files': image_files, 'gt': gt, 'name': video}

        return info

    def load_UAVTRACK112(self):
        info = {}
        base_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset)
        json_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset + '.json')
        jsons = json.load(open(json_path, 'r'))
    ...
    def load_UAVTRACK112L(self):
        info = {}
        base_path = join(realpath(dirname(__file__)), '../../dataset', 'UAVTRACK112')
        json_path = join(realpath(dirname(__file__)), '../../dataset', self.dataset + '.json')
        jsons = json.load(open(json_path, 'r'))
        videos = list(jsons.keys())
