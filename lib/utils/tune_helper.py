...
class SOTtuner_builder(nn.Module):
    ...
    def track_tune(self, siam_tracker, siam_net, video, hp):
        ...
        elif 'VISDRONE' in self.dataset or \
                'GOT10K' in self.dataset or \
                'NFS' in self.dataset or \
                'TC128' in self.dataset or \
                'UAV123' in self.dataset or \
                'DTB70' in self.dataset or \
                'UAV10FPS' in self.dataset or \
                'VISDRONE' in self.dataset or \
                'UAVDT' in self.dataset or \
                'UAVTRACK112' in self.dataset or \
                'UAV20L' in self.dataset:
            with open(result_path, "w") as fin:
                for x in boxes:
                    p_bbox = x.copy()
        ...
        if 'OTB' in self.dataset or \
                'VIS' in self.dataset or \
                'VOT' in self.dataset or \
                'GOT10K' in self.dataset or \
                'NFS' in self.dataset or \
                'TC128' in self.dataset or \
                'UAV123' in self.dataset or \
