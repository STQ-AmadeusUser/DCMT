...
def build_siamese_opt_lr(cfg, model, current_epoch=0):
    ...
    if current_epoch >= cfg.TRAIN.UNFIX_EPOCH:
        ...
        try:
            for layer in cfg.MODEL.BACKBONE.TRAINABLE_LAYER:
                for param in getattr(model.backbone, layer).parameters():
                    param.requires_grad = True
    ...
    if not cfg.MODEL.NECK['NAME'] == 'None':
...
def build_densesiam_opt_lr(cfg, model, current_epoch=0):
    '''
    common learning tricks in Siamese: fix backbone (warmup) --> unfix
    but except arch parameters
    '''
    for param in model.module.backbone.parameters():
        param.requires_grad = False
    for m in model.module.backbone.parameters():
        if isinstance(m, nn.BatchNorm2d):
            m.eval()
    ...
    trainable_params = []
    trainable_params += [{'params': filter(lambda x: x.requires_grad, model.module.backbone.parameters()),
                          'lr': cfg.TRAIN.LAYERS_LR * cfg.TRAIN.BASE_LR}]
    trainable_params += [{'params': model.module.neck.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
    trainable_params += [{'params': model.module.match.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
    trainable_params += [{'params': model.module.fuse.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
    trainable_params += [{'params': model.module.collector.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
    trainable_params += [{'params': model.module.prediction.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
...
def build_retrain_opt_lr(cfg, model, current_epoch=0):
    '''
    common learning tricks in Siamese: fix backbone (warmup) --> unfix
    but except arch parameters
    '''
    ...
    trainable_params += [{'params': model.match.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
    trainable_params += [{'params': model.fuse.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
    trainable_params += [{'params': model.collector.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
    trainable_params += [{'params': model.prediction.parameters(), 'lr': cfg.TRAIN.BASE_LR}]
