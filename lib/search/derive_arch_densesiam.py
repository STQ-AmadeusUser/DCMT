...
class ArchGenerate(BaseArchGenerate):
    ...
    def derive_archs(self, betas, head_alphas, stack_alphas, if_display=True):
        ...
        derived_archs = [
            [[3, self.config.MODEL.NECK.IN_CHANNEL], 'ResNet50Dilated'],
            [[self.config.MODEL.NECK.IN_CHANNEL, self.config.MODEL.NECK.OUT_CHANNEL], 'ShrinkChannel'],
            [[self.config.MODEL.NECK.OUT_CHANNEL, self.config.MODEL.NECK.OUT_CHANNEL], 'ROIMatch'],
        ]
